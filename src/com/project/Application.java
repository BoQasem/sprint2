...
import com.project.Discount.Offer;
import com.project.Discount.discounts;
import com.project.event.CaptainArrivedD;
import com.project.event.CaptainArrivedS;
import com.project.event.CaptainSetPrice;
import com.project.event.UserAcceptPrice;
...
//				***************************
		...
		Ride ride = new Ride(source, destination, "alimi", 1); 
        ...
        String eventTime = dateObj.format(myFormatObj);

        double price = 0.0; 
		
        events.add(new CaptainSetPrice(eventTime,price));
        events.add(new UserAcceptPrice(eventTime));

        
        events.add(new CaptainArrivedS(eventTime));
        events.add(new CaptainArrivedD(eventTime));
        //-------------------------------------------------------


        //---------------------Dicount---------------------------
        discounts obj = new discounts(new Offer());

        obj.getPriceAfterDescount(0.1);
        obj.showScreen("with a 10% discount for first ride");

		if(Driver.checkAreaDicounts("Dokki")){
			obj.getPriceAfterDescount(0.1);
			obj.showScreen("with a 10% discount for ride destination area");
		}

        obj.getPriceAfterDescount(0.05);
        obj.showScreen("with a 10% discount for ride contains two passengers");
        
        obj.getPriceAfterDescount(0.05);
        obj.showScreen("with a 10% discount for ride date is on a public holiday");
        
        obj.getPriceAfterDescount(0.1);
        obj.showScreen("with a 10% discount for ride date matches the birth date");
        //-------------------------------------------------------
